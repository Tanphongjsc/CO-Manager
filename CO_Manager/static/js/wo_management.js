document.addEventListener('DOMContentLoaded', () => {
    ...
    createNewBtn?.addEventListener('click', function () {
        window.location.href = '/wo/create/';
    });
    ...
    function initDeleteButtons() {
        document.querySelectorAll('.btn-delete').forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (confirm(`Bạn có chắc chắn muốn xóa bảng kê WO "${name}"?`)) {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            alert(data.message || 'Xóa bảng kê WO thành công');
                            window.location.href = window.location.pathname;
                        } else {
                            throw new Error(data.message || 'Không thể xóa bảng kê WO');
                        }
                    })
                    .catch(error => {
                        ...
                        this.disabled = false;
                        this.style.backgroundColor = originalBgColor || '#dc3545';
                    });
                }
            });
        });
    ...
    function initializeFromUrl() {
        ...
        if (orderParam) {
            ...
            // Set production order after options are loaded
            if (productionParam) {
                setTimeout(() => {
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            // Default behavior sẽ navigate đến URL trong href
            console.log('Navigating to detail page:', this.href);
        });
    });
});